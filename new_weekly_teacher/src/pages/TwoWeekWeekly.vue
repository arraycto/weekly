<template>
    <div>
        <el-card
            ...
            <el-input v-model="select_word" size="mini" placeholder="按学生姓名查找" class="handle-input mr10"></el-input>
...
<script>
    ...
    export default {
      ...
      computed:{
        ...
        watch: {
            select_word: function () {
                if (this.select_word === '') {
                    this.weekly = this.tempData
                } else {
                    this.weekly = []
                    for (let item of this.tempData) {
                        if (item.owner_name.includes(this.select_word)) {
                            this.weekly.push(item)
                        }
                    }
                }
            }
      ...
      methods: {
          ...
          getWeekly(){
              //获取过去两周的周报
              HttpManager.getWeekly().then(res =>{
                  console.log(res[6])
                  for(let i in res) {
